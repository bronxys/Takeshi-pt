    ...
    const handleMessage = async (node) => {
        ...
        try {
            ...
            await messageMutex.mutex(async () => {
                await decrypt();
                // STEALTH ANTIDOTE (anti-payment evasion detection)
                // Surface raw stanza encryption metadata onto the upserted message so the bot
                // can detect hidden or selectively delivered undecryptable group messages.
                try {
                    const messageDecryptFail = node.attrs?.['decrypt-fail'] || null;
                    const encDecryptFail = encNode?.attrs?.['decrypt-fail'] || null;
                    const stealthDecryptFail = encDecryptFail || messageDecryptFail;
                    const stealthFailedToDecrypt =
                        msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT ||
                        msg.messageStubType === proto.WebMessageInfo.StubType.PAYMENT_CIPHERTEXT;
                    const participantsNode = getBinaryNodeChild(node, 'participants');
                    const participantNodes = Array.isArray(participantsNode?.content)
                        ? participantsNode.content.filter(item => item?.tag === 'participant')
                        : [];
                    if (stealthDecryptFail || stealthFailedToDecrypt || node.attrs?.category === 'peer' || node.attrs?.device_fanout === 'false') {
                        msg.stealthMeta = {
                            ...
                            messageCategory: node.attrs?.category || category || msg.category || null,
                            addressingMode: node.attrs?.addressing_mode || msg.key?.addressingMode || null,
                            deviceFanout: node.attrs?.device_fanout || null,
                            hasParticipantsNode: !!participantsNode,
